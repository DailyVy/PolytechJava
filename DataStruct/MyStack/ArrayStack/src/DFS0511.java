...
public class DFS0511 { // DFS, BFS 실습
    // dfs(start 행, start 열, 입력할 배열, 방문했는지 확인, 채울 숫자
    static int dfs(int startRow, int startCol, int[][] input,
                    int[][] visit, int n_group) {
        // 여기서 바뀌면 메인에서도 바뀜
        // 2차원 배열이 복사되서 넘어가는게 아니고 레퍼런스가 넘어가서 그래
        // 확인해보자
        // visit[0][0] = 999;
        ...
        while(stack.empty() == false) {
            ...
            // pop 하고 방문처리가 안 되어있다면 count를 늘린다.
            if (visit[row][col] == 0) ++count;
    ...
    public static void main(String[] args) {
        ...
        // 단지의 가구 수를 출력하기 위해 가구 수를 넣은 리스트생성
        ArrayList<Integer> eachComplex = new ArrayList<>();
...
//                    System.out.println(i + " " + j + "에서 탐색 시작");
                    int apartNum = dfs(i, j, input, visit, ++n_group);
                    eachComplex.add(apartNum);
                    System.out.println(n_group + "단지의 아파트 수는 " + apartNum);
                }
            }
        }
        eachComplex.sort(Comparator.naturalOrder()); // 오름차순으로 정렬하기 위한 Comparator.naturalOrder()
        System.out.println(eachComplex);
        eachComplex.add(0, n_group); // 맨 앞에는 단지의 수
        System.out.println(eachComplex);
